...
| ---------------- | ------------------------------------------------------- | ----------------------------------- |
| [`api/`](api)    | Hono API on Cloudflare Workers (D1, R2, JWT auth)       | Worker `diuqbank`                   |
| [`web/`](web)    | Vite + React 19 SPA (React Router, TanStack Query)      | Workers static assets `diuqbank-web`|
| `shared/`        | `@diuqbank/shared`: response types, Zod schemas, utils  | (consumed by both — not deployed)   |

`shared/` is the single source of truth for the API↔web contract: the web imports the response
...
```bash
cp web/.env.example web/.env.local   # sets VITE_API_URL=http://localhost:8787
```
...
The API requires Worker secrets (`JWT_SECRET`, `ADMIN_EMAIL`, …) — see [api/README.md](api/README.md).
