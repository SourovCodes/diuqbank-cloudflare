...
import {
  ...
  coursesListQuery,
  courseUpdateSchema,
} from "../../shared/schemas/admin/courses";
...
route.delete("/:id", async (c) => {
  ...
  const [{ value: manualSubmissionCount }] = await db
    ...
    .where(eq(manualSubmissions.courseId, id));
  if (manualSubmissionCount > 0) {
    throw new HTTPException(409, {
      message: `Cannot delete: ${manualSubmissionCount} manual submission(s) reference this course`,
    });
  }
