...
export function QuestionReader({ question, submissions }: { question: QuestionDetail; submissions: PublicSubmission[] }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
      <Link href="/questions" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 transition hover:text-primary">
        <ArrowLeft className="size-4" /> Back to questions
      </Link>

      <header className="mt-5 border-b pb-7">
        <h1 className="max-w-5xl text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">{question.title}</h1>
        <p className="mt-2 text-muted-foreground">{question.course.name}</p>
      ...
      {!submissions.length ? (
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)_290px]">
          <aside className="order-2 xl:order-none">
            <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
              <div className="border-b px-4 py-3"><h2 className="text-sm font-bold">Submissions</h2></div>
          ...
          <section ref={viewer} className="min-w-0 overflow-hidden rounded-2xl border bg-white shadow-sm fullscreen:rounded-none">
            <div className="flex min-h-14 items-center justify-between gap-3 border-b bg-slate-50 px-3 py-2">
              ...
              <div className="flex items-center gap-1">
                ...
                {selected?.pdfUrl ? (
                  <a href={selected.pdfUrl} target="_blank" rel="noreferrer" aria-label="Download PDF" className="flex size-9 items-center justify-center rounded-lg text-slate-500 transition hover:bg-white hover:text-primary"><Download className="size-4" /></a>
                ) : null}
              </div>
            </div>
            {selected?.pdfUrl ? (
              <iframe title={`${question.title} PDF`} src={`${selected.pdfUrl}#toolbar=1&navpanes=0`} className={cn("h-[70vh] min-h-[560px] w-full bg-slate-100", fullscreen && "h-[calc(100vh-3.5rem)]")} />
            ) : (
          ...
          <aside className="order-3">
            <div className="rounded-2xl border bg-white p-5 shadow-sm">
              <h2 className="text-sm font-bold uppercase tracking-wide text-slate-500">Submission details</h2>
              <dl className="mt-5 grid gap-4 text-sm">
                <Detail label="Contributor" value={selected?.contributor?.name ?? "Anonymous"} icon={<UserRound className="size-4" />} />
              ...
              {selected?.pdfUrl ? (
                <a href={selected.pdfUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-white transition hover:bg-blue-700">
...
function Chip({ children, icon }: { children: React.ReactNode; icon?: React.ReactNode }) {
  return <span className="inline-flex items-center gap-1.5 rounded-full border bg-white px-3 py-1.5 text-slate-600">{icon}{children}</span>;
}

function IconButton({ children, label, disabled, onClick }: { children: React.ReactNode; label: string; disabled?: boolean; onClick: () => void }) {
  return <button type="button" aria-label={label} disabled={disabled} onClick={onClick} className="flex size-9 items-center justify-center rounded-lg text-slate-500 transition hover:bg-white hover:text-primary disabled:pointer-events-none disabled:opacity-30">{children}</button>;
}

function Detail({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return <div><dt className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-slate-400">{icon}{label}</dt><dd className="mt-1 font-medium text-slate-800">{value}</dd></div>;
}
